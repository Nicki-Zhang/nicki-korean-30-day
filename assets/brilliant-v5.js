...
function enhanceHeader(){
 ...
 const done=json('nicki-v3-ko-done'),streak=+(localStorage.getItem('nicki-v5-streak')||Math.max(1,Math.min(7,done.length))),xp=+(localStorage.getItem('nicki-v5-xp')||done.length*100);
...
function run(){enhanceHeader();enhanceHero();renderHome();renderCourses();restoreHero();enhanceDashboard();enhanceStage();enhanceLesson();syncLessonBar();setTimeout(syncLessonBar,60)}
new MutationObserver(run).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','class']});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run);else run();
})();