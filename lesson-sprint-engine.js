(function (global) {
  ...
  function render(){
    ...
    if(step.type==='complete'){
      const first=completeLesson();stage.innerHTML=`<div class="completeMark">✓</div><p class="tag center">${escape(tr(step.tag))}</p><h1 class="center">${escape(tr(step.title))}</h1><p class="lead center">${escape(tr(step.lead))}</p><div class="feedback good center"><b>${escape(first?ui('xpEarned'):ui('xpClaimed'))}</b><span>${escape(ui('saved'))}</span></div><div class="foot"><button class="primary" data-action="home">${escape(ui('returnCourses'))}</button></div>`;return;
    }
