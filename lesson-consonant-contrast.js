(function (global) {
  ...
  function renderQuestion(question, retry=false, embedded=false) {
    ...
    const optionButtons=options.map(option=>{let cls='';if(answer){if(answer.correct&&option===question.correct)cls='correct';else if(option===answer.choice)cls='wrong';}return `<button class="quizOption ${cls}" data-action="${retry?'retry-answer':'answer'}" data-question="${question.id}" data-value="${option}" ${answer||!audioReady?'disabled aria-disabled="true"':''}>${optionLabel(option)}${answer?.correct&&option===question.correct?' ✓':answer&&option===answer.choice?' ✕':''}</button>`;}).join('');
    const category=SOUND_CATEGORY[question.correct]||question.correct;
    const reveal=answer?.correct?`<div class="answerReveal"><b>${text('transcriptLabel')}：${player.transcript}</b><span>${text('categoryResult')}：${categoryLabel(player.category)}</span><span>${text('correctAnswerLabel')}：${optionLabel(player.correctAnswer)}</span><span>${text('teachingPointLabel')}：${categoryDetail(category)}</span></div>`:'';
    const feedback=answer?`<div class="feedback ${answer.correct?'good':'try'}" role="status"><strong>${text(answer.correct?'correct':'incorrect',{detail:answer.correct?categoryDetail(category):''})}</strong>${retry&&!answer.correct?`<button class="replayButton" data-action="retry-again" data-question="${question.id}">↻ ${text('retryAgain')}</button>`:`<button class="replayButton" data-action="question-audio" data-question="${question.id}" aria-label="${text('playerAriaReplay')}">▶ ${text('playerReplay')}</button>`}</div>`:'';
  ...
  function renderQuiz(questionId) {
    ...
    return `${renderQuestion(question)}<p class="note">${text('previewOnly')}</p>${footer({nextDisabled:!answered})}`;
  }
