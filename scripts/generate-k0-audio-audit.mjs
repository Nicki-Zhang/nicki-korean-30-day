...
for(const row of objects){const entry=entryFor(row);const playable=entry?audio.canPlayAudio(entry.speechText,entry,entry.audioType):false;row.catalogEntryId=entry?`${entry.lessonId}:${entry.id}`:'';row.catalogSpeechText=entry?.speechText||'';row.audioFilePath=entry?.path||'';row.fileExists=entry?String(fs.existsSync(entry.path)):'false';row.exactMatch=entry?String(entry.speechText===row.expectedSpeechText):'false';row.reviewStatus=entry?.reviewStatus||'missing';row.voiceSource=entry?.voiceSource||'';row.model=entry?.model||'';row.generationDate=entry?.generationDate||'';row.commercialUseBasis=entry?.commercialUseBasis||'';row.nativeReviewer=entry?.nativeReviewer||'';row.fallbackBehavior='none; fail closed with localized status';row.enabled=String(playable);if(playable&&entry.lessonId==='lesson-00')row.buttonText=entry.audioType==='vowel'?`听元音${entry.targetSymbol}`:`听示例音节 ${entry.speechText}`;row.serviceWorkerCached=playable&&cachedAssets.has(entry.path)?'true':row.serviceWorkerCached;row.issueType=playable?'released':entry?(fs.existsSync(entry.path)?'rights-and-review-metadata-gap':'missing-audio'):'missing-catalog-entry';row.priority=playable?'none':'P1';row.recommendedAction=playable?'Product-owner approved exact hosted asset; keep integrity checks active.':entry?'Keep disabled until approved, exact, technically validated, and available.':'Create an exact typed catalog record before generation.';}
...
const stats={players:objects.length,catalog:entries.length,physical:physical.length,activeFiles:activeExisting.length,missing:missing.length,pending:entries.filter(x=>x.reviewStatus==='pending').length,legacy:legacy.length,legacySystem:legacySystem.length,approved:approved.length,p0:0,p1:objects.filter(x=>x.enabled!=='true').length,p2:0,playable:entries.filter(x=>audio.canPlayAudio(x.speechText,x,x.audioType)).length};
...
fs.writeFileSync('K0_AUDIO_RIGHTS_AND_REVIEW_GAPS.md',`# K0 音频权利与审核缺口\n\n${summary}\n当前54条音频已完成技术验证和产品负责人课程内试听；nativeReviewStatus=deferred、nativeReviewer=null。其 rightsReviewStatus 均继续为 pending，与当前课程接入分离，不伪造法律结论。其余${stats.catalog-stats.approved}条尚无完整发布记录。16条 legacy-system-export 仍须完成商业权利复核；其中已试听通过的记录也不代表商业发布批准。\n`);
// The batch plan is a reviewed allowlist document, not a generated audit summary.
// Do not overwrite it when refreshing the player/file audit outputs.
fs.writeFileSync('K0_NATIVE_AUDIO_REVIEW_CHECKLIST.md',`# K0 产品负责人试听与母语抽检清单\n\n${summary}\n产品负责人逐条核对 displayText、speechText、audioType、预期读音、自然度、音变、剪切、音量和语境韵律。产品负责人通过后记录 reviewMethod、reviewedAt 和技术验证证据；不得伪造 nativeReviewer。母语者审核为可选发布前抽检；왜/예的 syllable 与 word 版本须分别试听。\n`);

const queueHeader='| id | lessonId | targetSymbol | displayText | speechText | audioType | pronunciationType | pronunciationRule | file | voiceSource | model | voice | generationDate | commercialUseBasis | rightsReviewStatus | reviewStatus | assetStatus | reviewMethod | reviewedAt | technicalValidation | sha256 | sourceRunId | sourceArtifact | nativeReviewStatus | nativeReviewer | reviewNotes |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|';
const queueRows=entries.map(x=>`| ${x.id} | ${x.lessonId} | ${x.targetSymbol??''} | ${x.displayText} | ${x.speechText} | ${x.audioType} | ${x.pronunciationType} | ${x.pronunciationRule} | ${x.file} | ${x.voiceSource} | ${x.model??''} | ${x.voice??''} | ${x.generationDate??''} | ${x.commercialUseBasis??''} | ${x.rightsReviewStatus??''} | ${x.reviewStatus} | ${x.assetStatus} | ${x.reviewMethod??''} | ${x.reviewedAt??''} | ${x.technicalValidation??''} | ${x.sha256??''} | ${x.sourceRunId??''} | ${x.sourceArtifact??''} | ${x.nativeReviewStatus??''} | ${x.nativeReviewer??''} | ${x.reviewNotes} |`);
