...
assert.ok(api);
assert.equal(api.lessonId,'lesson-07','Lesson 7 must use its own current course identity.');
...
const before={xp:200,completedLessons:['lesson-06'],lessonProgress:{'lesson-06':100,'lesson-07':31},guest:true};
const first=api.completionPatch(before);
assert.equal(first.xp,250);
assert.equal(first.lessonProgress['lesson-07'],100);
assert.ok(first.completedLessons.includes('lesson-07'));
...
assert.equal(lesson.stableId,'lesson-07');
...
const canonicalLesson4=fs.readFileSync('lesson-04.html','utf8');
assert.match(canonicalLesson4,/lesson-consonant-contrast\.js/);
