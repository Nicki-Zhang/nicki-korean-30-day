...
for(const marker of ['--brand-primary:#6d4aff','--brand-hover:#5638d4','--brand-active:#452bb5','--brand-soft:#f4f0ff','--brand-border:#ddd3ff','--brand-focus:rgba(109,74,255,.48)','min-height:44px','prefers-reduced-motion'])assert.ok(shellCss.includes(marker),`Purple dashboard token or accessibility rule missing: ${marker}`);
for(const marker of ['--friendly-brand-600:var(--brand-primary)','--friendly-brand-500:#9c83ff','--friendly-gradient:linear-gradient','--friendly-text-korean','--friendly-radius-xl','--friendly-duration-normal','min-height:44px','prefers-reduced-motion'])assert.ok(friendlyCss.includes(marker),`Friendly learning path token or accessibility rule missing: ${marker}`);
...
assert.match(shellCss,/linear-gradient\(128deg,#5638d4 0%,#6d4aff 54%,#9c83ff 100%\)/,'Dashboard hero must use the approved aligned purple gradient.');
