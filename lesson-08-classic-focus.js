(function (global) {
  ...
  const STEP_CONTENT_TYPES = Object.freeze({
    ...
    'group-choice': 'choice',
    'linking-rule': 'explanation',
    'link-choice': 'choice',
    'link-match': 'matching',
    'nasal-rule': 'explanation',
    'nasal-choice': 'choice',
    'recognize-rule': 'explanation',
    'rule-choice': 'choice',
    'rule-match': 'matching',
    'final-check': 'choice',
    complete: 'completion'
  ...
  const STEP_TYPES = Object.freeze({
    ...
    'group-match': 'match',
    'group-choice': 'choice',
    'linking-rule': 'concept',
    'link-choice': 'choice',
    'link-match': 'match',
    'nasal-rule': 'concept',
    'nasal-choice': 'choice',
    'recognize-rule': 'concept',
    'rule-choice': 'scenario',
    'rule-match': 'match',
    'final-check': 'choice',
    complete: 'complete'
  });
  ...
  if (!config || config.id !== LESSON_ID) throw new Error('Lesson 8 Classic Focus adapter requires the explicit Lesson 8 configuration.');
  for (const step of config.steps) {
    if (!Object.hasOwn(STEP_CONTENT_TYPES, step.id)) throw new Error(`Lesson 8 Classic Focus has no explicit renderer mapping for ${step.id}.`);
    if (STEP_TYPES[step.id] !== step.type) throw new Error(`Lesson 8 step type changed for ${step.id}.`);
  }
  ...
  function learningPathUrl() {
    return `nikigo-app.html?lang=${encodeURIComponent(activeLanguage)}&learnStage=K0&learnModule=k0-batchim-and-sound-changes#courses`;
  }

  function update(viewModel) {
    if (viewModel.lessonId !== LESSON_ID) throw new Error('Lesson 8 adapter received another lesson identity.');
    const contentType = STEP_CONTENT_TYPES[viewModel.stepId];
    if (!contentType || STEP_TYPES[viewModel.stepId] !== viewModel.stepType) throw new Error(`Unsupported Lesson 8 step mapping: ${viewModel.stepId}`);
